define(["require", "exports", "./ng_app"], function (require, exports, ng_app_1) {
    ...
    ng_app_1.ng_app.controller("TransSimCtrl", ['$scope', '$interval', '$timeout', '$window', '$http', '$sce',
        function ($scope, $interval, $timeout, $window, $http, $sce) {
            //time
            $interval(function () { $scope.nowtime = new Date(); }, $interval(function () {
            ...
            $scope.search = function () {
                if ($scope.switcha) {
                    ...
                    $http(reqmater).then(function (response) { $scope.materials = response.data; }, function () { alert('err'); });
                }
                if ($scope.switchb) {
                    $scope.b = false;
                    $scope.bb = true;
                    ...
                    $http(reqship).then(function (response) { $scope.ships = response.data; }, function () { alert('erro'); });
                }
                if ($scope.switchc) {
                    $scope.c = false;
                    $scope.cc = true;
                    ...
                    $http(reqbase).then(function (response) { $scope.bases = response.data; }, function () { alert('err'); });
                }
            };
            //搜索平台-----------------------------
            $scope.sendobj =
            ...
            $scope.showmaterial = function (materialname, materialdetail, materialid, materialtype, materialvolume, materialweight, materialamount) {
                ...
                $scope.addship = function () {
                    ...
                    var flag = 1;
                    for (var _i = 0, _a = $scope.selected_materials; _i < _a.length; _i++) {
                        var num = _a[_i];
                        if ($scope.id == num.id) {
                            alert('物资不可重复添加');
                            var flag = 0;
            ...
            $scope.edit = function () {
                ...
                $scope.labelsc.splice(0, $scope.labelsc.length);
                $scope.datac[0].splice(0, $scope.datac[0].length);
                for (var _i = 0, _a = $scope.sendobj.material; _i < _a.length; _i++) {
                    var ma = _a[_i];
                ...
                if ($scope.sendobj.ship == 0) {
                    ...
                    for (var _b = 0, _c = $scope.ships; _b < _c.length; _b++) {
                        var ship = _c[_b];
                    ...
                    for (var _d = 0, _e = $scope.sendobj.ship; _d < _e.length; _d++) {
                        var ma = _e[_d];
            ...
            // }
            //将物资和数量存入数组-----------------------------
            var Materobjplus = /** @class */ (function () {
                function Materobjplus() {
                }
                return Materobjplus;
            }());
            $scope.matervalues = [];
            ...
            $scope.showship = function (shipid, shipname, shipinfo, shipnation, shipport, shipseazone, shipweight) {
                ...
                $scope.addship = function () {
                    if (pointer_flag == false) {
                        ...
                        var flag = 1;
                        for (var _i = 0, _a = $scope.sendobj.ship; _i < _a.length; _i++) {
                            var num = _a[_i];
            ...
            $scope.showbase = function (baseid, storage_place, base_x, base_y) {
                ...
                $scope.addship = function () {
                    ...
                    // else{
                    var flag = 1;
                    for (var _i = 0, _a = $scope.sendobj.base; _i < _a.length; _i++) {
                        var num = _a[_i];
            ...
            $scope.showplatform = function () {
                ...
                $scope.addship = function () {
                    if (pointer_flag == false) {
                        ...
                        var flag = 1;
                        for (var _i = 0, _a = $scope.sendobj.platform; _i < _a.length; _i++) {
                            var num = _a[_i];
                            if (1 == num.platformid) {
                                alert('平台不可重复添加');
                                var flag = 0;
            ...
            $scope.fangzhen = function () {
                ...
                if ($scope.sendobj.ship == 0) {
                    for (var _i = 0, _a = $scope.ships; _i < _a.length; _i++) {
                        var ship = _a[_i];
                        $scope.sendobj.ship.push({ shipid: ship.id, x: null, y: null, shipweight: ship.deckweight });
                    }
                }
                ...
                //           {alert('suc')},
                //            function(){alert('err')});
                $http.post('http://' + IpAdress + ':8888/ssh/schedule/parse', $scope.sendobj).then(function (response) {
                    console.log(response), $scope.responseid = response.data.replace('\r\n', ''),
                        // $http({
                        //     method:'GET',
                        ...
                        $http({
                            ...
                            params: { no: $scope.responseid }
                        }).then(function (response) { console.log(response), $scope.modalBody = $sce.trustAsHtml(response.data); }, function () { alert('sec err'); });
            ...
            $scope.clear_selected = function () {
                for (var _i = 0, _a = $scope.map.points; _i < _a.length; _i++) {
                    var obj = _a[_i];
                    obj.selected = false;
                }
            };
